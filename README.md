...
Bonjour à tous ! Je m'appelle Alexandre et je suis heureux de vous présenter le projet Qulbutoquiz que j'ai développé en une semaine.
...
## Pourquoi avoir développé ce site Web ?

J'ai développé ce site Web dans le cadre d'un test de technicité sur le langage, Ruby on Rails demandé par la société StudyCall.

## Pourquoi avoir développé avec Ruby on Rails (RoR) ?

J'avais le choix du langage de programmation même si le faire en Ruby était fortement conseiller. J'aurais pu le programmer dans des langages que je connaissais et que je maîtrisais mieux comme React JS ou Symfony, mais je voulais découvrir ce langage donc je me suis lancé dans la découverte de Ruby on Rails. A ma grande surprise, Ruby fut très facile à apprendre avec mes connaissances en C et python, je ne fus pas dé familiarisé.
...
Malgré tout, je m'y suis retrouvé avec mes bases en Symfony et j'ai apprécié utilisé RoR .

Maintenant que vous en savez plus sur le projet et moi, passons à la mise en place du projet pour le tester localement.

# Logiciel pré réquis

Il y a trois logiciels à installer :
...
> L'installation de phpmyadmin est<b> optionnelle</b> sauf s'il vous est nécessaire d'accéder à la base de données

> Dans ce cas-là, une fenêtre apparaitra trois fois, sélectionner<b> Apache 2</b> lors de la première, <b>Oui</b> sur la seconde et rentrer un <b>mot de passe </b>sur la troisième.
...
Afin de pouvoir **créer, mettre à jour, supprimer et lire** sur la **base de donnés** ; RoR a besoin d'un **compte** avec des accès à celle-ci.

Pour créer ce compte, on accède à la console **mysql**.

    sudo mysql

On exécute les requêtes suivantes , pour **créer l'utilisateur** puis lui **donner les droits** sur la futue base de données et enfin **rendre actif** les privilèges ajoutés.
...
>Dans le cas où vous voulez phpmyadmin voilà la démarche à suivre
...
Notre utilisateur <b>`ruby`</b> maintenant crée, on peut lancer la **création** de la **base de données** par Rails
...
La base de données est déjà pourvue en question et quiz pour jouer directement.

Elle est également pourvue d'un compte **admin** qui a les permissions pour modifier, supprimer et créer des quiz.
...
Le joueur peut choisir un quiz et devra répondra à **5 questions.**
...
Le **joueur** a la possibilité de se **connecter** depuis l'onglet log in grâce à son nom d'utilisateur et son mot de passe.

S'il n'en a pas, il a la possibilité d'en **créer** un avec l'aide de okéoké.

Une fois connecté, le joueur peut accéder à ces informations de son compte depuis l'onglet account.

Il peut a tout moment se **déconnecte** via l'icone **logout**

L'outil de **création** de question est pour l'instant très **instable** et ne peut créer des questions uniquement avec des réponses de type image et des questions textes.

# Conclusion


Ce projet fut une **rude épreuve** car je devais apprendre un langage avec lequel je devais produire un projet en une semaine. L'épreuve fut du moins **lucratif**, j'ai appris beaucoup du langage RoR au cours de cette semaine, j'ai compris des **méthodologies** à appliquer au bon fonctionnement du site. Si je devais le recommencer à zéro, j'aurai une **manière de construction** complètement **différente** : beaucoup plus **découpé** mon **code** et éviter les gros **blocs**.
